...
If you upload PCK files at different hosting server, make sure that the CDN allows you download such file (especially CORS policy), then you need to change hostname to that location by using `Api.host()` function, chained with `http.get_pck()` as usual:

```gdscript
	var http := Api.host("http://cdn.of.new.site/").http_get_pck("pck/pck_2022_12_12_patch.pck")
	yield(http, "completed")
	get_tree().change_scene("res://pck_2022_12_12_patch/new_scene.tscn")
```
